...
# we can normalize by dividing by 255
# Data is in interger so convert into floating
X, X_test = X.astype('float32')/255.0, X_test.astype('float32')/255.0
...
# 20% of nodes are set to 0
model.add(Dropout(0.2))
#now we add another convolution layer, again with a 3x3 kernel
#This time our padding=valid this means that the output dimension can
#take any form
model.add(Conv2D(32, (3, 3), activation='relu', padding='valid'))
# maxpoolwith kernel of 2x2
model.add(MaxPooling2D(pool_size=(2, 2)))
# In CNN ,we need to flatten our data before we can input it into output/dense layer
model.add(Flatten())
# Dense layer with 512 hidden units
model.add(Dense(512, activation='relu'))
# this time set 30% to 0 to minimize overfitting
model.add(Dropout(0.3))
# Finally output dense layer with 10 hidden units corr to 10 classes
model.add(Dense(10, activation='softmax'))
